...
func CreateEventStoreTestClient(t *testing.T) *client.Client {
  config, err := client.ParseConnectionString(fmt.Sprintf("esdb://admin:changeit@%s?tlsverifycert=false", "localhost:2113"))
	if err != nil {
...
func TestReadStreamEventsForwardsFromZeroPosition(t *testing.T) {
  client := CreateEventStoreTestClient(t)
	defer client.Close()
