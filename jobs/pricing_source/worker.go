...
func init() {
	app.RegisterJobsPricingSourceInterface(func(a *app.App) tjobs.PricingSourceJobInterface {
		return &PricingSourceJobInterfaceImpl{a}
	})
}

type PricingSourceJobInterfaceImpl struct {
	App *app.App
}

func (m *PricingSourceJobInterfaceImpl) MakeWorker() model.Worker {
	worker := Worker{
		name:      JobName,
